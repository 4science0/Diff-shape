...
class SamplingMetrics(nn.Module):
    def __init__(self, train_smiles, dataset_infos, test, template=None, template_name=None, test_smiles=None, filter=False):
        super().__init__()
        ...
        self.test = test
        self.template, _, = get_template_sdf(template, dataset_infos, template_name) if template else None
    ...
    def evaluate(self, generated, local_rank):
        ...
        print(f"Validity * Uniqueness: {validity*uniqueness*100 :.2f}%")
        if self.test_smiles is not None:
            print(f"test_smiles_novelty: {self.test_smiles_novelty.compute() * 100 :.2f}%")

        if wandb.run:
            dic = {'Validity': validity,
                   'Uniqueness': uniqueness,
                   'Novelty': novelty,
                   'Validity*Uniqueness': validity*uniqueness,
    ...
    def __call__(self, molecules: list, name, current_epoch, local_rank, sample_type="mixed_sample"):
        ...
        all_generated_smiles, all_valid_mols, result_dict = self.evaluate(molecules, local_rank=local_rank)
        ...
        filename = f"{result_path}/final_smiles_GR{local_rank}_{0}"
        sdf_filename = f"{result_path}/final_mol_GR{local_rank}_{0}"
        for i in range(2, 10):
            if os.path.exists(filename):
                filename = f"{result_path}/final_smiles_GR{local_rank}_{i}"
        ...
        if len(all_valid_mols.keys())>0:
            ...
            for idx in all_valid_mols.keys():
                ...
                with Chem.SDWriter(f"{sdf_filename}_template{idx}.sdf")as f:
                    f.write(template)
                ...
                smiles_f.close()
            result_df.to_csv(f"{result_path}/result_metric.csv", mode='a', index=False, header=False, float_format='%.2f')
            ...
            #     idx = template.GetProp('template_idx')
            #     with Chem.SDWriter(f"{sdf_filename}_template{idx}.sdf")as f:
            #         f.write(template)
            #         for sample_i, mol in enumerate(all_valid_mols[idx]):
            #             mol.SetProp('_Name', f"sample{sample_i}")
            #             f.write(mol)
            # print(f'All mol saved on rank {local_rank}')
...
if __name__ == '__main__':
    ...
    def main(cfg: omegaconf.DictConfig):
        ...
        sampling_metrics = SamplingMetrics(train_smiles=train_smiles, dataset_infos=dataset_infos, test=True,
                                           template=datamodule.test_dataset.template_data, test_smiles=test_smiles)
