...
class ChemblDrugsDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        if self.remove_h:
            ...
            chembl_mols = Chem.SDMolSupplier(self.raw_paths[0], removeHs=False)
        # chembl_mols = list(chembl_mols)[75400:]
        chembl_mols = list(chembl_mols)
        ...
        print(f"chembl len(data_list):{len(data_list)}")
        ##geom##
        ...
        for i, smiles in enumerate(tqdm(all_files)):
            ...
            for j, conformer in enumerate(all_conformers):
                ...
                data_list.append(data)
            all_smiles.append(smiles)
        print(f"geom len(data_list):{len(data_list)}")
        # ###########
